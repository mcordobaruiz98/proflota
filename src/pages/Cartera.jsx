...
function Cartera({ viajes = [], vehiculos = [], onEditar, mostrarToast }) {
  ...
  viajesFiltrados.forEach(v => {
    ...
    porEmpresa[emp].total += (v.saldoPendiente ?? v.vViaje ?? 0);
  });
  ...
  const totalPendiente = pendientes.reduce((s, v) => s + (v.saldoPendiente ?? v.vViaje ?? 0), 0);
  const vencidos = pendientes.filter(v => calcVencimiento(v).vencido);
  const totalVencido = vencidos.reduce((s, v) => s + (v.saldoPendiente ?? v.vViaje ?? 0), 0);
                ...
                >
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginLeft: "10px" }}>
                    <span style={{ fontSize: t.fonts.sizeSm, fontWeight: t.fonts.weightBold, color: pagado ? t.colors.green : t.colors.textPrimary }}>
                      {fmt(v.saldoPendiente ?? v.vViaje ?? 0)}
                      {v.anticipoMonto > 0 && (
                        <span style={{fontSize:"9px",color:t.colors.textTertiary,display:"block"}}>
                          Flete {fmt(v.vViaje||0)} · Anticipo {fmt(v.anticipoMonto)}
                        </span>
                      )}
                    </span>
